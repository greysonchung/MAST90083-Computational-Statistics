...
Compute_IC <- function(model, n, IC1, IC2, IC3) {
  for (p in P) {
    ...
    coefficient <- ginv(t(tmp_X) %*% tmp_X) %*% t(tmp_X) %*% matrix(tmp_y)
...
# Table of counts of the selected model by IC1, IC2 and IC3
IC_count <- data.frame(IC_count, row.names = c("IC1", "IC2", "IC3"))
colnames(IC_count) <- c(1:10)
IC_count
```
...
n_samples <- 15
M1_set_15 <- matrix(0, n_samples, 1000)
for (i in 1:1000) {
  M1 <- rep(0, n_samples)
  for (j in 6:n_samples) {
    M1[j] <- 0.434 * M1[j-1] + 0.217 * M1[j-2] + 0.145 * M1[j-3] + 0.108 * M1[j-4] + 
...
IC_count <- matrix(0, 3, 10)
for (i in 1:1000) {
  IC <- Compute_IC(M1_set_15[, i], n_samples, rep(0, 10), rep(0, 10), rep(0, 10))
  IC_count[1, which(IC[,1] == min(IC[,1]))] <- IC_count[1, which(IC[,1] == min(IC[,1]))] + 1
  IC_count[2, which(IC[,2] == min(IC[,2]))] <- IC_count[2, which(IC[,2] == min(IC[,2]))] + 1
  IC_count[3, which(IC[,3] == min(IC[,3]))] <- IC_count[3, which(IC[,3] == min(IC[,3]))] + 1
}
...
n_samples <- 100
M2_set_100 <- matrix(0, n_samples, 1000)
for (i in 1:1000) {
  M2 <- rep(0, n_samples)
...
IC_count <- matrix(0, 3, 10)
for (i in 1:1000) {
  IC <- Compute_IC(M2_set_100[, i], n_samples, rep(0, 10), rep(0, 10), rep(0, 10))
  IC_count[1, which(IC[,1] == min(IC[,1]))] <- IC_count[1, which(IC[,1] == min(IC[,1]))] + 1
  IC_count[2, which(IC[,2] == min(IC[,2]))] <- IC_count[2, which(IC[,2] == min(IC[,2]))] + 1
  IC_count[3, which(IC[,3] == min(IC[,3]))] <- IC_count[3, which(IC[,3] == min(IC[,3]))] + 1
}
...
# Generate 1000 sets of size 15 using model M1
n_samples <- 15
M2_set_15 <- matrix(0, n_samples, 1000)
for (i in 1:1000) {
  M2 <- rep(0, n_samples)
  for (j in 3:n_samples) {
    M2[j] <- 0.682 * M2[j-1] + 0.346 * M2[j-2] + rnorm(1)
...
IC_count <- matrix(0, 3, 10)
for (i in 1:1000) {
  IC <- Compute_IC(M2_set_15[, i], n_samples, rep(0, 10), rep(0, 10), rep(0, 10))
  IC_count[1, which(IC[,1] == min(IC[,1]))] <- IC_count[1, which(IC[,1] == min(IC[,1]))] + 1
  IC_count[2, which(IC[,2] == min(IC[,2]))] <- IC_count[2, which(IC[,2] == min(IC[,2]))] + 1
  IC_count[3, which(IC[,3] == min(IC[,3]))] <- IC_count[3, which(IC[,3] == min(IC[,3]))] + 1
}
