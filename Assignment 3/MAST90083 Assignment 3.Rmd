...
$$
Z_j = f_j(\beta_{0j}+\sum_{m=1}^{r}\beta_{mj}\cdot x_m)
$$
The output node $Y_k$ can be express as follow, where $g_k$ is the activation function for the output layer
$$
Y_k = g_k[\alpha_{0k} + \sum_{j=1}^{t}\alpha_{jk}\cdot f_j(\beta_{0j}+\sum_{m=1}^{r}\beta_{mj}\cdot x_m)]
$$

## Question 2.2
The hidden layer can be express as follow using matrix form, where $f$ is the activation function of the hidden layer, $\mathbf{Z} = (Z_1,...,Z_t)^\top$ is a vector of the hidden nodes, $\mathbf{\beta_0} = (\beta_{01},...,\beta_{0t})^\top$ is a vector of biases of the hidden nodes, $\mathbf{B} = (\beta_{1},...,\beta_{t})^\top$ is a matrix with $(t\times r)$ rows of connection weight for each hidden nodes, and $\mathbf{X} = (X_1, ..., X_r)^\top$ is matrix of the input.
$$
\mathbf{Z} = f(\mathbf{\beta_0} + \mathbf{X}^\top\mathbf{B})
$$
The output layer can be express as follow using matrix form, where $g$ is the activation function of the hidden layer, $\mathbf{Y} = (Y_1,...,Y_t)^\top$ is a vector of the output nodes, $\mathbf{\alpha_0} = (\alpha_{01},...,\alpha_{0s})^\top$ is a vector of biases of the hidden nodes, $\mathbf{\alpha} = (\alpha_{1},...,\alpha_{s})^\top$ is a matrix with $(s\times t)$ rows of connection weight for each output nodes, and $\mathbf{Z}$ is described in the above function.
$$
\mathbf{Y} = g(\mathbf{\alpha_0} + \mathbf{\alpha}\mathbf{Z})
$$

## Question 2.3
If the activation function $f_j()$ fo the hidden layer is simply the identity function, and the actiavtion function $g_k()$ is the same as the one of the single-layer perceptron, then the two-layer network will be equivalent to a single layer perceptron.
